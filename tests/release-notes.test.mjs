...
test('GitHub Releases include beginner-facing install, update, and uninstall instructions before publish', () => {
  ...
  assert.match(releaseWorkflow, /npx -y @deepseek-ai\/dsh@0\.1\.0-rc\.8 plugin --profile web add dsh-codex-subscription/u)
  assert.match(releaseWorkflow, /dsh@0\.1\.0-rc\.8 plugin --profile web add dsh-codex-subscription/u)
  assert.match(releaseWorkflow, /dsh@0\.1\.0-rc\.8 plugin --profile web update dsh-codex-subscription/u)
  assert.match(releaseWorkflow, /dsh@0\.1\.0-rc\.8 plugin --profile web remove dsh-codex-subscription/u)
