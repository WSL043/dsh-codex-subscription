...
test('shipped agent guide owns install, pinned update, verification, and uninstall', () => {
  ...
  assert.match(guide, /dsh plugin --profile web add dsh-codex-subscription@1\.1\.2/u)
  assert.match(guide, /\.\\dsh\.exe plugin --profile web add dsh-codex-subscription@1\.1\.2/u)
  assert.match(guide, /npx -y @deepseek-ai\/dsh@0\.1\.0-rc\.8 plugin --profile web add dsh-codex-subscription@1\.1\.2/u)
...
test('Windows manager updates from a checksum-verified immutable release asset', () => {
  ...
  assert.match(manager, /\$PackageVersion = '1\.1\.2'/u)
  assert.match(manager, /\$PackageSpec = 'dsh-codex-subscription@1\.1\.2'/u)
