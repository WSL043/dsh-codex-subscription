...
test('public readmes provide explicit update commands and verification', () => {
  ...
  assert.match(readmeZh, /## 更新[\s\S]*-Action Update[\s\S]*dsh plugin --profile web add https:\/\/github\.com\/WSL043\/dsh-codex-subscription\/releases\/download\/v0\.2\.3\/[\s\S]*dsh plugin --profile web list[\s\S]*dsh --profile web --dump-config/u)
  assert.match(readmeEn, /## Update[\s\S]*-Action Update[\s\S]*dsh plugin --profile web add https:\/\/github\.com\/WSL043\/dsh-codex-subscription\/releases\/download\/v0\.2\.3\/[\s\S]*dsh plugin --profile web list[\s\S]*dsh --profile web --dump-config/u)
