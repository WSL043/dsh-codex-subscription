...
test('GitHub defaults to concise Chinese and directs Agents to their own guide', () => {
  ...
  assert.match(readme, /DSH-Portable（社区便携包）[\s\S]*github\.com\/deepseek-ai\/deepseek-harness#run/u)
  assert.match(readmeEn, /community DSH-Portable package[\s\S]*github\.com\/deepseek-ai\/deepseek-harness#run/u)
