...
if (command === 'list') {
  ...
  packages['dsh-codex-subscription'] = { version: process.env.DSH_CODEX_TEST_ADDED_VERSION || '1.1.1' }
...
windowsTest('legacy manager can install into a completely new profile with an empty list response', () => {
  ...
  try {
    ...
    assert.equal(installed['dsh-codex-subscription'].version, '1.1.1')
  } finally {
...
windowsTest('update fails instead of reporting success when DSH keeps an older package version', () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0)
    assert.match(result.stderr, /expected 1\.1\.1[^]*found 0\.2\.8/iu)
...
windowsTest('installed portable mode expands its external state root', () => {
  ...
  try {
    ...
    assert.equal(plan.arguments.includes('dsh-codex-subscription@1.1.1'), true)
