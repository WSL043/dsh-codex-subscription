...
`;
		...
		function SketchStudio({ open, agentEnabled, agentPreview, onOpen, onClose, attachSketch, enabled, t, incoming, sessionId, rpc, sessionState }) {
			...
			const save = async (name) => {
				...
				try {
					await sketchDrafts("save", row, sessionId);
				} catch (error) {
			...
			const saveChanges = async () => {
				...
				if (hasContent() || saved.current) await save();
				else {
					await sketchDrafts("clearRecovery", sessionId);
					dirty.current = false;
					setRecovered(false);
				}
			};
			...
			const load = async (row) => {
				...
				row = await sketchDrafts("get", row.id);
			...
			(0, react.useEffect)(() => {
				...
				restore().catch(() => {
					if (live) setError(t("sketchStorageFailed"));
				}).finally(() => {
					if (live) {
						setHydrated(true);
						setBusy(false);
			...
			const close = async () => {
				...
				try {
					...
					setError(error.code?.startsWith("SKETCH_") ? error.message : t("sketchStorageFailed"));
				} finally {
					setBusy(false);
				}
			};
			...
			const attach = async () => {
				...
				try {
					...
					setError(error.code?.startsWith("SKETCH_") ? error.message : t("sketchFailed"));
				} finally {
					setBusy(false);
				}
			};
