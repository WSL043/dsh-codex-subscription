...
`;
		...
		function paintSketch(context, strokes, size = SKETCH_SIZE, transparent = false, height = size) {
		...
		function paintSketchLayers(context, doc, cache, size = doc.width ?? 1024, height = doc.height ?? size) {
			context.globalCompositeOperation = "source-over";
			context.globalAlpha = 1;
			...
			for (const layer of doc.layers) {
				...
				if (surface.width !== size || surface.height !== height) {
					surface.width = size;
					surface.height = height;
				}
		...
		function SketchStudio({ open, onClose, attachSketch, enabled, t }) {
			...
			const paint = () => {
				...
				paintSketchLayers(canvas.current.getContext("2d"), doc.current, cache.current);
			};
			const schedule = () => {
			...
			const move = (event) => {
				...
				for (const sample of events.length ? [...events, native] : [native]) {
					...
					if (gesture.eraseStroke) {
						...
						})).some((p) => strokeHit(stroke, p, width / 2, doc.current.width, doc.current.height)));
					} else {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("canvas", {
							style: { "--sketch-ratio": (doc.current.width ?? 1024) / (doc.current.height ?? 1024) },
							ref: canvas,
							width: SKETCH_SIZE,
							height: SKETCH_SIZE,
							"aria-label": t("sketchTitle"),
