...
`;
		...
		function connectSketchAgent(rpc, sessionId, execute, report) {
		...
		function SketchStudio({ open, onClose, attachSketch, enabled, t, incoming, sessionId, rpc }) {
			...
			(0, react.useEffect)(() => {
				...
				const disconnect = rpc && sessionId ? connectSketchAgent(rpc, sessionId, api.execute, (message) => setError(message)) : void 0;
				return () => {
					disconnect?.();
