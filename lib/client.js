...
`;
		...
		function paintSketchLayers(context, doc, cache, size = doc.width ?? 1024, height = doc.height ?? size, activeLayer, images = /* @__PURE__ */ new Map()) {
			context.globalCompositeOperation = "source-over";
			context.globalAlpha = 1;
			...
			for (const layer of doc.layers) {
				...
				if (entry.count !== count || entry.prefix !== prefix || entry.image !== layer.image || entry.strokes !== layer.strokes) {
					...
					paintSketch(ctx, layer.strokes.slice(0, count), size, true, height);
					entry.count = count;
					entry.prefix = prefix;
				...
				if (moving) {
					...
					paintSketch(ctx, layer.strokes.slice(-1), size, true, height);
		...
		function createStrokeFilter(strength = 0) {
			...
			return (point, timestamp) => {
				...
				const dt = Math.max(1 / 1e3, Math.min(.1, (timestamp - time) / 1e3));
				...
				filtered = {
					x: filtered.x + a * (point.x - filtered.x),
					y: filtered.y + a * (point.y - filtered.y)
				};
				last = point;
				time = timestamp;
				return filtered;
		...
		function SketchStudio({ open, onClose, attachSketch, enabled, t }) {
			...
			const [stability, setStability] = (0, react.useState)(0);
			...
			const move = (event) => {
				...
				for (const sample of events.length ? [...events, native] : [native]) {
					...
					if (!point) continue;
					if (gesture.filter) point = gesture.filter(point, sample.timeStamp);
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("canvas", {
							...
							onPointerDown: (event) => {
								...
								const filter = tool === "pen" ? createStrokeFilter(stability) : null;
								filter?.(start, event.timeStamp);
								active.current = {
									id: event.pointerId,
									layer: layer.id,
									eraseStroke,
