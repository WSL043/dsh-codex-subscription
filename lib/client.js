		...
		*/
		...
		function createPreferenceController(scope, rpc) {
			...
			const refreshModels = async () => {
				...
				if (hadError) publish();
				try {
					const value = unwrap$1(await rpc.call(CHANNEL$1, "preferences/models", {}));
...
`;
		...
		function ContextWindowPreference({ preference, t }) {
			...
			(0, react.useEffect)(() => setDrafts(Object.fromEntries(modelRows.map((model) => [model.key, String(snapshot.customContextWindows[model.key])]))), [modelRows.map((model) => `${model.key}\u0000${snapshot.customContextWindows[model.key]}`).join("")]);
