...
const SUPPORTED_RUNTIME_VERSIONS = /* @__PURE__ */ new Set([
	"0.1.5-rc.2",
	"0.1.5-rc.3",
	"0.1.7-rc.1"
]);
...
function matchingSubagentRuntimeVersion(resolve = require.resolve) {
	try {
		...
		return SUPPORTED_RUNTIME_VERSIONS.has(host.version) ? host.version : void 0;
	} catch {
		return;
	}
}
function inspectSubagentRuntime(resolve = require.resolve) {
	try {
		...
		if (!SUPPORTED_RUNTIME_VERSIONS.has(manifest.version) || manifest.version !== matchingSubagentRuntimeVersion(resolve)) return {
			installed: false,
			present: true
		};
