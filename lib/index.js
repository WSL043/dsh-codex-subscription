...
function requiredSpanMs(consumedPercent, resolution = 1) {
	if (resolution < 1 && consumedPercent >= Math.max(.05, resolution * 2)) return 2 * 6e4;
...
function estimateQuotaForecast(state, window, now = Date.now(), context = {}) {
	...
	if (samples.length >= 2) {
		...
		if (consumed >= 1 && spanMs >= 6e4 && now - last.at <= 20 * 6e4 && (samples.length < MIN_SAMPLES || spanMs < requiredSpanMs(consumed))) {
	...
	const resolution = samples.reduce((step, sample) => {
		for (const candidate of [
			...
			.001,
			1e-4
		]) if (Math.abs(sample.remainingPercent / candidate - Math.round(sample.remainingPercent / candidate)) < 1e-6) return Math.min(step, candidate);
		return step;
	}, 1);
	if (consumedPercent > 0 && consumedPercent < (resolution === 1 ? 1 : Math.max(.05, resolution * 2))) return {
		status: "calibrating",
		reason: "resolution",
		sampleCount: samples.length
	};
	if (spanMs < requiredSpanMs(consumedPercent, resolution)) return {
		status: "calibrating",
		sampleCount: samples.length,
		observedSpanMs: spanMs,
		consumedPercent
	...
	if (recent.length >= 3 && last.at - recent[0].at >= 5 * 6e4) {
		...
		if (recentPace > pacePerHour * 2 || recentPace < pacePerHour / 2) {
			if (samples.length > recent.length) return estimateQuotaForecast({ windows: {
				...state.windows,
				[keyFor(window, context)]: {
					...record,
					samples: recent
				}
			} }, window, now, context);
