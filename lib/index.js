	...
	return async (endpoint, payload, signal) => {
		if (endpoint === "preferences/status" || endpoint === "preferences/update") try {
			...
			if (endpoint === "preferences/update") {
				if (typeof payload?.sidebarQuotaVisible !== "boolean") return publicError("internal", "Invalid sidebar quota preference");
...
function apply(ctx) {
	const settings = ctx.settings.register(settingsNamespace(SETTINGS_NAMESPACE), z.object({ [SIDEBAR_QUOTA_FIELD]: z.boolean().default(true) }));
	const preferences = {
		status: () => ({
...
//#endregion
export { CODEX_USAGE_URL, CodexLoginCoordinator, DshOAuthCredentialStore, apply, assertCodexAuthUrl, commandForCodexAuthUrl, createCodexAuthService, createCodexRpcHandler, createCodexUsageReader, createSubscriptionRpcHandler, inject, name, openCodexAuthUrl, parseCodexUsage };
