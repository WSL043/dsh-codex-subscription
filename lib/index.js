...
//#endregion
//#region src/image-gallery.js
function sessionImageGallery(events) {
			...
			for (const item of result.content ?? []) {
				...
				images.set(ref.attachmentId, {
					attachment: {
						...
						width: ref.width,
						height: ref.height,
						name: typeof ref.name === "string" ? ref.name.slice(0, 128) : "image.png"
					},
					original
				});
			}
...
function estimateQuotaForecast(state, window, now = Date.now(), context = {}) {
	...
	const uncertaintyPerHour = Math.max(deviations.length === 0 ? 0 : median(deviations) * 1.4826, .5 / (spanMs / HOUR_MS));
	const recent = samples.filter((sample) => sample.at >= last.at - 30 * 6e4);
	if (recent.length >= 3 && last.at - recent[0].at >= 5 * 6e4) {
		const recentPace = (recent[0].remainingPercent - last.remainingPercent) / ((last.at - recent[0].at) / HOUR_MS);
		if (recentPace > pacePerHour * 2 || recentPace < pacePerHour / 2) return {
			status: "calibrating",
...
function createSubscriptionRpcHandler({ authHandler, usageReader, resetCreditService, preferences, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, getImageGallery }) {
	return async (endpoint, payload, signal) => {
		if (endpoint === "image/gallery") {
			...
			return value === void 0 ? publicError("not-ready", "Session images are not ready yet") : {
				ok: true,
				value
			};
		}
