...
- Use the pinned `v0.2.6` release assets for a first install, never a moving branch.
...
```powershell
curl.exe -fL https://github.com/WSL043/dsh-codex-subscription/releases/download/v0.2.6/dsh-codex.ps1 -o "$env:TEMP\dsh-codex.ps1"
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri 'https://github.com/WSL043/dsh-codex-subscription/releases/download/v0.2.6/dsh-codex.ps1' -OutFile "$env:TEMP\dsh-codex.ps1"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$env:TEMP\dsh-codex.ps1" Install
```
