...
- Use the exact `1.12.2` package below; do not install a moving branch.
...
```sh
npx -y @deepseek-ai/dsh@0.1.2-rc.1 plugin --profile web add dsh-codex-subscription@1.12.2
```

With an existing global `dsh` command:

```sh
dsh plugin --profile web add dsh-codex-subscription@1.12.2
```
...
1. `dsh-codex-subscription` version `1.12.2` appears exactly once.
