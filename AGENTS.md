...
- Use the pinned `v0.3.5` release assets for a first install, never a moving branch.
...
```powershell
curl.exe -fL https://github.com/WSL043/dsh-codex-subscription/releases/download/v0.3.5/dsh-codex.ps1 -o "$env:TEMP\dsh-codex.ps1"
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri 'https://github.com/WSL043/dsh-codex-subscription/releases/download/v0.3.5/dsh-codex.ps1' -OutFile "$env:TEMP\dsh-codex.ps1"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$env:TEMP\dsh-codex.ps1" Install
```
...
```sh
dsh plugin --profile web add dsh-codex-subscription@0.3.5
```
