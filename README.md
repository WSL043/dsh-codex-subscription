...
**功能开关：** 设置 → Codex 订阅 → 图片工作台，可分别控制新图生成、参考图编辑、`@生图`、查看器、标注、草图、模板、图库、对比。关闭快捷入口不影响自然语言生图；同时关闭生成和编辑后，后续模型请求不再携带图片工具。关闭功能保留历史图片。
...
![图片功能独立开关](docs/assets/image-settings-2.0.png)
