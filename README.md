...
设置仅保留“显示额度”和“续航预测 · 实验”。输入框只展示一个 28px 图标；悬停查看剩余摘要，点击查看各周期、重置时间及启用后的续航预测。旧百分比/进度条设置仍兼容读取，但不再对应不同的外观。

续航预测需主动开启，仅出现在详情中；关闭额度显示会暂停采样。预测至少需要 3 个样本，持续高消耗时通常需 5–10 分钟，低消耗时会延长观察。最近 24 小时的非敏感观测保存在本机，额度重置、账号切换或关闭功能会重新校准。
