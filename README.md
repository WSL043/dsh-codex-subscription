...
| **可选额度恢复** | 可选择让对话等待已确认的短时 Codex 额度重置；对话会显示等待原因、仍可停止，等待期间切换账号会立即重试 |
...
包名不带版本号时安装最新正式版。指定版本时填写 `dsh-codex-subscription@2.1.9`；测试版则使用对应发布说明中的完整版本号。此处只填包名，不要粘贴整条终端命令。安装本插件使用上面的 npm 包名即可，无需填写 GitHub 地址或本地目录。
...
- **Codex 独立子任务**：复用订阅登录和官方 DSH Codex 运行时，无需另行登录；可选组件在高级设置中安装。默认继承当前订阅模型及工作区权限。开启 DSH 子任务模型选择、配置允许模型并新建会话后，可在聊天中指定子任务模型与推理档位；非订阅会话需明确选择订阅模型。共享上下文的子任务仍交给 DSH。
...
优先使用上面的 **安装组件** 按钮。正式版 **2.1.9** 会按当前 DSH 版本选择对应组件：DSH `0.1.7-rc.2` 安装组件 `0.1.7-rc.2`，DSH `0.1.5-rc.2` 安装组件 `0.1.5-rc.2`；已安装的 `0.1.5-rc.3` 组件仅在同版本宿主上识别。请不要省略组件版本号或自行改用 `@next`。
...
只想停用：在高级设置切回 **DSH**，组件仍保留，之后可再次开启。
