...
可在设置中选择关闭、百分比、进度条或 Beta 续航预测；紧凑额度只在选择 Codex 模型时显示。续航预测仅在用户主动选择后，根据官方剩余百分比估算当前消耗速度。它至少需要 3 个样本；持续高消耗时通常 5–10 分钟即可给出范围，消耗较低时会延长观察或显示稳定。最近 24 小时的无敏感信息观测会保存在本机，重启后可以继续校准；额度重置、账号切换或关闭功能会开启新的校准周期。输入框按周期显示简短剩余百分比，如“5h 80% · 周 54%”，不合并不同周期，也不虚构未返回的额度。进度条模式仍保留数字；点击可展开全部周期、重置时间及启用后的续航预测，触屏和键盘均可操作。
