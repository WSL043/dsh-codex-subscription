...
> 当前工作分支为 **2.0.0 验收候选版，尚未发布**。本轮整理整个订阅插件的账号、额度、设置、模型和通信模块，并加入 `@Sketch` 草图 Beta、输入框图片指令和生成结果预览编辑。设置 → Codex 订阅 → 高级与诊断 → 图片可分别控制各功能，以及图片模型和质量。2.5 型号为实验选项，默认仍为 GPT Image 2。详见 [架构与维护边界](docs/2.0.0-architecture.md) 和 [实机验收记录](docs/2.0.0-acceptance.md)。
...
<p align="center">
  <img src="docs/assets/sketch-2.0.png" width="900" alt="Codex 订阅直接用在 DSH：订阅模型、联网搜索、额度与安全重置、图片生成和高速模式">
</p>
...
- 额度提醒可关闭、使用预设，或分别自定义 5 小时与周／长周期的剩余阈值（1–100%）；不使用超过五分钟的旧数据发出提醒。
...
| `@生图` / `@Image` | 在输入框写清生成或编辑要求 | 原位插入图片指令，保留已有说明与引用；手动发送 |
| `@Sketch` / `@草图`（Beta） | 用线条或构图说明想法 | 打开画板，确认只附加参考图 |

快捷入口和自然语言可以同时使用，不需要每次输入 `@`。草稿始终可见，已有说明不会被替换，也不会因打开面板而自动发送或生成。

草图提供钢笔、铅笔、荧光笔，支持局部擦除和整笔擦除；最多 8 个图层，可新建、复制、重命名、隐藏／显示、上下排序和删除。撤销／重做覆盖绘画及图层操作，导出只合成可见图层。画板使用合并指针采样、曲线平滑和逐帧绘制；草图保存在当前工作台内存中，刷新前请附加到草稿。总笔画上限 200，单笔采样会压缩以限制内存。

![草图与图层](docs/assets/sketch-layers-2.0.png)

**功能开关：** 设置 → Codex 订阅 → 高级与诊断 → 图片，现在只保留三组：图片生成与编辑、创作入口、图片浏览。隐藏创作入口不影响自然语言生图；关闭图片生成与编辑后，后续模型请求不再携带图片工具。图片浏览可选择增强模式或 DSH 默认。已有细分设置不一致时显示“保留原设置”，主动更改该组才会统一调整。关闭功能保留历史图片。

**上下文与用量：** `@` 是明确意图的入口，不是免上下文或免用量模式。画板与未发送的草稿不作为新消息加入对话；发送后，创作说明、参考图和图片结果仍会占用会话上下文。连续做大量图片迭代时，建议使用独立会话。Luna 等对话模型负责调用工具，图片引擎独立选择。

![图片创作入口](docs/assets/image-create-2.0.png)
![图片设置（浅色）](docs/assets/image-settings-2.0.png)
![图片设置（深色）](docs/assets/image-settings-2.0-dark.png)
...
<p align="center">
  <img src="https://raw.githubusercontent.com/WSL043/dsh-codex-subscription/main/docs/assets/composer-2.0-dark.png" width="800" alt="中文 DSH 输入框内的额度图标">
</p>

输入框额度保留横向四种模式：关闭、百分比、进度条、续航预测。百分比直接显示周期与余额；进度条只显示短周期标签（如 5h、周）与条形，不重复百分比；续航模式显示预测或校准状态。悬浮与点击共用紧凑详情层，每周期一行，展示剩余比例和重置时间；启用预测时补充简短结果。

设置页只保留“账号与偏好”和“高级与诊断”：账号、额度和提醒偏好在同一页；搜索、上下文、独立图片设置卡及诊断位于高级页。单独一个周额度时，进度条不再带文字，完整周期及百分比可展开查看。

续航预测使用最近两小时的样本，避免整天空闲稀释近期消耗。缓存回复不会重复采样，过期数据不用于估算；长时间断档、额度重置和速度突变会重新校准。详情显示包含额度报告精度误差的估计范围。这仍是按近期速度的估算，不保证未来任务消耗；保留 Beta，后续应通过真实使用回放评估误差后再决定转正。

额度提醒在新鲜数据的剩余比例不高于所选阈值时触发：只让当前模型的输入框额度变红；额度回升后恢复，不在设置页或弹层重复提醒。


账号读取失败时可重试或清除本插件登录信息。清除需要在界面确认，会退出本插件保存的全部账号，保留会话和设置；服务断开时需先恢复 DSH。服务端诊断失败或超时后，仍可复制包含插件版本、时间及安全错误分类的本地报告，不包含凭据、账号身份或带认证参数的网址。
...
图片菜单只保留生成／编辑与草图 Beta。已移除创作模板和图片库：参考图直接粘贴或拖入 DSH 输入框；插件生成结果仍可点开预览、标注并继续编辑。`@生图` / `@Sketch` 不会自动发送。

![紧凑输入框](docs/assets/composer-2.0-dark.png)

![高级页的图片设置](docs/assets/subscription-images-2.0-dark.png)

参考图的 5 张限制是当前订阅工具的本地保护上限，不是 GPT 图片模型的通用上限。官方 [Images API](https://developers.openai.com/api/reference/resources/images/methods/edit) 支持最多 16 张，但订阅通道尚未完成相同数量的实测，因此没有直接扩大并宣称支持。

额度解析保留上游小数；本次账号观测只返回整数。预测对真实小数样本可在至少三个样本、两分钟且消耗足够时估算；整数数据不能凭空细化。短暂停顿不清空历史，速度变化优先使用最近半小时重估。官方 [App Server](https://learn.chatgpt.com/docs/app-server) 另有 token 活动统计，但没有提供 token 与订阅剩余额度的固定换算。
