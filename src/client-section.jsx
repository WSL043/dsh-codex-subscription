...
export function CodexSection({ preference, rpc, accountStatus, t }) {
  ...
  const id = useId()
  const tabs = ['account', 'display', 'advanced']
  ...
  return <section className="codexSubscription">
    ...
    <div role="tabpanel" id={`${id}-display`} aria-labelledby={`${id}-display-tab`} hidden={tab !== 'display'}><PreferencesCard preference={preference} t={t} /></div>
