...
export function SketchWorkspace({ preference, attachSketch, registerOpen, t, sessionId, rpc }) {
  ...
  return <>
    <SketchStudio onOpen={() => { opener.current = document.activeElement; setOpen(true) }} sessionId={sessionId} rpc={rpc} incoming={incoming} open={open} onClose={() => { setOpen(false); opener.current?.focus() }} attachSketch={attachSketch} enabled={settings.imageSketch && settings.imageEditing} t={t} />
  </>
}
