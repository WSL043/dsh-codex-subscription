...
export function SketchFiles({ save, load, fresh, importImage, download, hasContent, disabled, t, report, onWorking }) {
  ...
  const run=async operation=>{setWorking(true);onWorking(true);try{await operation()}catch(error){report(error?.message||t('sketchStorageFailed'))}finally{setWorking(false);onWorking(false)}}
  ...
  return <div ref={host} className="codexSketchFiles" onKeyDown={e=>{if(e.key==='Escape'&&open){e.preventDefault();e.stopPropagation();setOpen(false)}}}>
    ...
    {open?<section className="codexSketchFilePanel" aria-label={t('sketchFiles')}>
      ...
      <div className="codexSketchExportFormats codexSketchSegment" role="group" aria-label={t('sketchExportFormat')}>
        {[['png','PNG'],['psd','PSD'],['draft',t('sketchEditableFile')]].map(([value,label])=><button key={value} type="button" aria-pressed={format===value} disabled={working} onClick={()=>setFormat(value)}>{label}</button>)}
      </div>
      <button type="button" disabled={working||!hasContent} onClick={()=>void run(async()=>{await download(format);setOpen(false)})}>{t('sketchDownload')}</button>
